...
def generate_description(aid_name: str, step_list: list) -> str:
    """
    計算式ステップ群を解析し、仕様書用の自然な日本語概要（1〜2文）に変換する。
    - ネスト条件構造（第N条件レベル：真/偽）を展開して前提条件を日本語化
    - 算術演算チェーン（加算・減算）を集約して一文に整理
    """
    if not step_list:
    ...
    def parse_levels(cond: str) -> dict:
        """'第1条件レベル：真 第2条件レベル：偽' → {1: True, 2: False}"""
        result = {}
    ...
    cond_labels = {}   # {level: description}  条件レベルが真/偽のとき何を意味するか
    calc_ops    = []   # [{levels, ptype, desc}]  実際の計算ステップ
    ...
    def clean_cond(label: str, truth: bool) -> str:
        """'全日休暇なら真（補足）' + truth=False → '全日休暇ではない'"""
    ...
    if has_nested and active:
        ...
        # 非ネスト構造: 「条件あり」の実行条件を直接使用
        # [---] / --- などの「常に実行（条件なし）」マーカーは除外する
        NO_COND_MARKER = re.compile(r'^\[?-+\]?$')  # "---", "[---]", "-" など
        for s in step_list:
            c = s.get("cond", "").strip()
    ...
    for op in active:
        ...
        # 一時変数の左辺参照を除去: 「一時変数0-特0休暇時間」→「-特0休暇時間」
        cleaned = re.sub(r'^一時変数\d+\s*', '', raw).strip()
        if not cleaned or '一時変数' in cleaned:
            continue

        # ptype が演算子の場合（加算/合算/減算）→ desc をそのまま値として使う
        if ptype in ('加算', '合算'):
            if base_val is None:
                base_val = cleaned
            else:
...
def process(content: str):
    ...
    def fkey(sc, aid):
        # 実行順序でソート後、(実行条件, 処理タイプ, 正規化された説明) で比較
        # 実行順序番号の違いは無視し、計算内容が同一なら同一バリアントと判定
...
def build_excel(data: dict) -> io.BytesIO:
    ...
    # 固定列: No / ID / 項目名称 / 単位 / 概要 / 備考  (A〜F)
    N_FIXED = 6
    # パターン列: 集約設定コードを1コード1列で配置  (G〜)
    glabels = [sc_map[sc] for sc in sc_list]
    N_TOTAL = N_FIXED + len(glabels)

    # ── 列幅 ──────────────────────────────────────────────────────────
    col_widths = [5, 10, 20, 6, 70, 14]   # No, ID, 項目名称, 単位, 概要, 備考
    ...
    c = ws.cell(1, 1, "リアルタイム算出パターンデータ一覧")
    ...
    notes = [
        ...
        "■ 概要列は計算ロジックを日本語で要約。同じIDで複数行ある場合はパターンごとに計算内容が異なる。",
    ]
    ...
    for ci, h in enumerate(["No", "ID", "項目名称", "単位", "概要", "備考"], 1):
    ...
    # ── データ行 ──────────────────────────────────────────────────────
    # A〜D（No/ID/項目名称/単位）は同一 集約値ID の複数バリアント行で縦結合
    # E〜（概要/備考/パターン列）はバリアントごとに1行ずつ
    cur = H2 + 1
    no  = 1
    ...
    for aid in aid_list:
        ...
        # A〜D: No / ID / 項目名称 / 単位（先頭行に入力後、複数行なら縦結合）
        # ※行高さは後のステップ数確定後に各行ごとに設定する
        ...
        for vi_idx, vn in enumerate(all_variants):
            ...
            # ステップ数に合わせた行高さ（1ステップ約15px、最低30px）
            ws.row_dimensions[r].height = max(30, len(step_list) * 15 + 4)

            put(ws, r, 5, summary)   # E: 概要
            put(ws, r, 6, "")        # F: 備考
